...
pub fn left(text: &str, length: usize) -> String {

    if len(text) <= length {
        return text.to_string();
    }
    return text.chars().take(length).collect();
}
...
pub fn middle(text: &str, start: usize, length: usize) -> String {
    return text.chars().skip(start).take(length).collect::<String>()
}
...
pub fn pad(text: &str, length: usize) -> String {
    ...
    let actual_length = len(text);

    if actual_length >= length {
        return format!("{}", text);
    }
    return format!("{}{}", text, " ".repeat(length - actual_length));
}
...
pub fn right(text: &str, length: usize) -> String {
...
pub fn heading_bar(value: &str, fill: bool) {
    ...
    let trailing_stars = fill_char.repeat(HEADING_WIDTH - 6 - length(value));
    println!(
        "{}──  {}  {}{}",
...
pub fn title_bar(value: &str, fill: bool) {
    let fill_char = if fill { "─" } else { " " };
    let trailing_stars = fill_char.repeat(TITLE_WIDTH - 5 - length(value));
