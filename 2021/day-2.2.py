...
def forward(position, value):
	return Position(position.distance + value, position.depth + value * position.aim, position.aim)

def up(position, value):
	return Position(position.distance, position.depth, position.aim - value)
